      ...
      linhasSped.forEach(linha => {
        ...
        if (colunas[1] === '0000') {
          ...
          const pFormatado = (dtIni.length === 8 && dtFin.length === 8) 
            ? `${dtIni.substring(0,2)}/${dtIni.substring(2,4)}/${dtIni.substring(4,8)} a ${dtFin.substring(0,2)}/${dtFin.substring(2,4)}/${dtFin.substring(4,8)}` : `${dtIni} a ${dtFin}`;
          let c = colunas[7] || '';
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={dadosGraficoOperacoes} cx="50%" cy="50%" innerRadius={80} outerRadius={120} paddingAngle={5} dataKey="value" animationDuration={1500}>
                        {dadosGraficoOperacoes.map((entry, index) => <Cell key={`cell-op-${index}`} fill={CORES_OPERACOES[index % CORES_OPERACOES.length]} />)}
                      </Pie>
                    ...
                    <div style={{ maxHeight: '280px', overflowY: 'auto', paddingRight: '5px' }}>
                      {listaCfops.entradas.length > 0 ? listaCfops.entradas.map((item, idx) => (
                        <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px', borderBottom: '1px solid #f0f4f8', fontSize: '14px' }}>
                          ...
                          <span style={{ color: '#10b981', fontWeight: '600' }}>{formatarMoeda(item.valor)}</span>
                        </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', borderLeft: '1px solid #f0f4f8', paddingLeft: '20px' }}>
                    ...
                    <div style={{ maxHeight: '280px', overflowY: 'auto', paddingRight: '5px' }}>
                      {listaCfops.saidas.length > 0 ? listaCfops.saidas.map((item, idx) => (
                        <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px', borderBottom: '1px solid #f0f4f8', fontSize: '14px' }}>
                          <span style={{ fontWeight: 'bold', color: '#555' }}>{item.cfop}</span>
                          <span style={{ color: '#4f46e5', fontWeight: '600' }}>{formatarMoeda(item.valor)}</span>
                        </div>
                  ...
                  <div style={{ maxHeight: '150px', overflowY: 'auto' }}>
                    {guiasE116.length > 0 ? guiasE116.map((guia, index) => (
                      <div key={index} style={{ backgroundColor: '#f8fafc', padding: '12px', borderRadius: '10px', marginBottom: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div><span style={{ fontSize: '12px', color: '#666', display: 'block' }}>Cód: {guia.codigo}</span><span style={{ fontSize: '12px', color: '#555' }}><Calendar size={12}/> Vencto: {guia.vencimento}</span></div>
                        <span style={{ fontWeight: 'bold', color: '#ef4444', fontSize: '16px' }}>{formatarMoeda(guia.valor)}</span>
                      </div>
                    )) : <p style={{ fontSize: '13px', color: '#999', textAlign: 'center' }}>Sem guias (E116).</p>}
                  </div>
                </div>
                ...
                <div className="print-card" style={{ backgroundColor: '#fff', padding: '25px', borderRadius: '20px', boxShadow: '0 10px 30px rgba(0,0,0,0.05)', width: '100%' }}>
                  ...
                  <div style={{ maxHeight: '150px', overflowY: 'auto' }}>
                    {ajustesICMS.length > 0 ? ajustesICMS.map((ajuste, index) => (
                      <div key={index} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px', borderBottom: '1px solid #f0f4f8', fontSize: '14px' }}>
                        <div><span style={{ display: 'block', fontWeight: 'bold', color: '#333' }}>{ajuste.descricao}</span><span style={{ fontSize: '11px', color: '#666' }}>Cód: {ajuste.codigo}</span></div>
                        <span style={{ fontWeight: 'bold', color: '#004080' }}>{formatarMoeda(ajuste.valor)}</span>
                      </div>
                    )) : <p style={{ fontSize: '13px', color: '#999', textAlign: 'center' }}>Nenhum ajuste (E111).</p>}
                  </div>
                </div>
              </div>
            </div>
